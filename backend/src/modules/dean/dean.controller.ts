...
import { createDean, deleteDean, getDean, updateDean } from './dean.service';
...
export const getDeanHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const page = parseInt(req.query.page as string) || 1;
        const limit = parseInt(req.query.limit as string) || 25;

        const filter: any = {};
        if (req.query.deanId) filter.deanId = parseInt(req.query.deanId as string);
        ...
        const deanList = await getDean(page, limit, filter);
...
export const updateDeanHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const deanId = parseInt(deanIdParam, 10);
        if (Number.isNaN(deanId)) {
            return next(new AppError('Invalid deanId', 400));
        }
